...
# [cite_start]16. Calculate average Salary per year and find the year with the highest average [cite: 182]
print("\n--- 16. Average Salary by Joining Year ---")
avg_salary_per_year = df.groupby(df['Joining_Date'].dt.year)['Salary'].mean()
avg_salary_per_year.index.name = 'Year'
highest_avg_year = avg_salary_per_year.idxmax()
print(avg_salary_per_year.round(2))
print(f"\nYear with the highest average salary: {highest_avg_year}")